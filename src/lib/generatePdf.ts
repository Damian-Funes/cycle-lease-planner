...
function addHeader(doc: jsPDF, params: SmartCycleParams) {
  ...
  // Add logo
  try {
    doc.addImage(LS_LOGO_BASE64, "PNG", 14, 10, 25, 14);
  } catch {
    // fallback: text logo
    doc.setFontSize(14);
  ...
  // Build right column lines dynamically
...
export function generateProposalPdf(params: SmartCycleParams, projection: YearProjection[]) {
  ...
  autoTable(doc, {
    ...
    footStyles: { fillColor: GRAY_LIGHT as any, textColor: BLACK as any, fontStyle: "bold", fontSize: 10 },
    didParseCell(data) {
      // Make TOTAL 10 ANOS row green bold
